# context-index Specification

## Purpose

The context-index capability composes the three upstream capability outputs (raw schema, semantic descriptions, relationship graph) into an immutable `ContextBundle` and persists it as per-capability JSON files under a bundle directory. It is the entrypoint for the `sonar scan` CLI and the read source for downstream consumers (MCP server).
