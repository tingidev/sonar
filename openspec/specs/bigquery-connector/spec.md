# bigquery-connector Specification

## Purpose

BigQuery data source adapter. Connects to GCP projects using Application Default Credentials, enumerates datasets/tables/columns via the BigQuery REST API, discovers FK/PK constraints via per-dataset INFORMATION_SCHEMA, and samples rows with `SELECT * LIMIT N`. Provides the same observable contract as the Postgres, Snowflake, and DuckDB connectors so downstream consumers (`relationship-mapping`, `description-engine`, `context-index`, `mcp-server`) operate on BigQuery output without branching.
