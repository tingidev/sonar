...
- The description engine's `medium` bucket is intentionally a "plausible PII" catch-all. Treating it as protected means we fail toward safety when the classifier is uncertain. Operators who verify the classifier is overcautious in their domain set `--allow-pii` — an informed opt-out with a clear audit trail (the flag is visible in the serve invocation).
