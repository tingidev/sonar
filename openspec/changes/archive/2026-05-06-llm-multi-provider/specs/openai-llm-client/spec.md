...
- **WHEN** `OpenAIClient(config).generate("summarise this table", system="You are a data analyst.")` is awaited with `config.model = "gpt-4o"`
- **THEN** the client SHALL call `AsyncOpenAI.chat.completions.create` with `model="gpt-4o"`, `max_tokens=config.max_tokens`, and messages containing the system prompt and user prompt
- **AND** it SHALL return `response.choices[0].message.content`

#### Scenario: Generation without system prompt

- **WHEN** `OpenAIClient(config).generate("hello")` is awaited without a system argument
...
- **AND** `OpenAIClient(config)` is constructed
